$(function() {
	...
	win.keydown(event => {
		...
		// When the client hits ENTER on their keyboard - Doubles for the Login Page as well as the Message Page...
		if (event.which === 13) {
			if (username) {
				sendMessage();
  ...
  // This captures the current action on the inputMessage field.
	inputMessage.on('input', () => {
		updateTyping();
	});
	
	// Capture the mouse click with a focus.
	loginPage.click(() => {
		currentInput.focus();
	});
